...
export function QrScanner({
  ...
  useEffect(() => {
    ...
    async function startCamera() {
      if (!navigator.mediaDevices?.getUserMedia) {
        setMessage(`이 브라우저에서는 카메라를 사용할 수 없습니다. ${manualLabel}을 이용해 주세요.`);
        return;
      }
  ...
  }, [cameraMessage, manualLabel, requestedDeviceId]);
    ...
    >
      <section className="qr-scanner-modal" role="dialog" aria-modal="true" aria-labelledby="qr-scanner-title">
        ...
        <form className="qr-manual-form" onSubmit={submitManualCode}>
          <label htmlFor="qr-manual-code">{manualLabel}</label>
          <div>
            <input
              id="qr-manual-code"
              value={manualCode}
            ...
            />
            <button type="submit" disabled={!manualCode.trim()}>확인</button>
          </div>
        </form>
      </section>
    </div>
